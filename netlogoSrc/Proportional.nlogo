...
to distribute-drivers-proportionally
  ask intersections [
    ...
    if total-capacity = 0 [stop];skips the last node, which has no outbound links
      
    ask my-out-links [
      let proportion capacity / total-capacity
      set opt-num-drv floor (opt-num-drv + (proportion * drv-at-intersection)) ;rounds down to avoid rounding errors (1 driver is added later)
    ]
    
    ;if the ratio of number of drivers here for total capacity is not integer, adds 1 driver to a random out link
    let ratio drv-at-intersection / total-capacity
    if  ratio != abs ratio [
      ask one-of my-out-links [
        set opt-num-drv opt-num-drv + 1
      ]
    ]
...
to choose-next-road 
  ask drivers [
    ...
    ;chooses an road from current intersection
