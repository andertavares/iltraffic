...
\begin{algorithm}
\caption{RL for route choice algorithm}
\label{alg:RLforRouteChoice}
\begin{algorithmic}

\item \{* Initialization *\}
\State $\epsilon \gets 1$
\State $ODpair \gets random\_OD\_pair$
\State $CurrentNode \gets Origin$
\State $P \gets \phi$ \Comment{$P$ is the driver's route}
\State $P^* \gets shortestPath(ODPair)$ \Comment{$P^*$ is the driver's shortest route}
\item \{* Execution *\}

\While{$CurrentNode \neq Destination$}
  \If{$random() \leq \epsilon$}
      \State $CurrentLink \gets RandomOutboundLink$
  \Else
      \State $CurrentLink \gets LinkWithMaxQ$
  \EndIf
  \State $P \gets P \cup \{CurrentLink\}$
  \State $CurrentNode \gets destinationOf(CurrentLink)$
\EndWhile

\item Calculate travel time for route $P$

\For{$l \in P$}
  \item Update Q table for $l$
\EndFor
\end{algorithmic}
\end{algorithm}

\subsubsection{Initialization:}
At the beginning of execution, OD pairs are randomly distributed among drivers. Then, each driver calculates the shortest route $P_i^*$ for his OD pair. As the costs of all links are the same, the shortest route is the one with less links between origin and destination. Drivers also initialise the exploration coefficient ($\epsilon$) at 1. This means that drivers will explore in the beginning to gain knowledge about the road network.
...
At step 1 the driver chooses an outbound link to traverse according to the $\epsilon$-greedy action selection strategy: choose an arbitrary link with probability $\epsilon$, or choose the best link according to the Q-table with probability $1-\epsilon$. At step 2, the destination node of the chosen link is reached. Step 3 tests whether the node reached in step 2 is the driver's final destination. If so, the trip ends, otherwise steps 1 and 2 are repeated. At step 4, drivers update their Q-tables on the entries regarding the links they traversed.

and experience their travel times given by Eq. \eqref{eq:tt}.

Then, at step 3, drivers update their Q-tables using the Q-learning update formula (Eq. \eqref{eq:qlearning}), where $Q(a)$ is the Q-value for action `choosing link $a$', $\alpha$ is the learning rate, $\gamma$ is the discount factor and $R$ is the reward received by the driver for traversing link $a$. 
...
%TODO explain why only Q(a) instead of Q(s,a) // just adopt Q(s,a) ...?
 ...
 When the trip ends, each driver $i$ calculates the travel time $\att{i}$ experienced on it's route $\route{i}$. It is given by the sum of the travel times experienced on each link of $\route{i}$. Eq. \eqref{eq:att} illustrates this, with $\veh$ being the number of vehicles on link $l$.
...
From a global point of view, it is desired that vehicles get distributed proportionally to the capacity of each link on the network. Road network load balance will be measured in two forms: number of congested links ($n$) and average overload ($o$). Considering $C$ as the set of congested links, $n$ these metrics will be measured according to Eq. \eqref{eq:balanceMetrics}, where $\veh_l$ and $\linkCap$ are the number of vehicles and the capacity of link $l$, respectively. 
...
The two metrics are needed because $n$ does not measures how heavy are the links congestions and $o$ does not shows how many links are overloaded. For both ($n$) and ($o$), smaller values means better performance, as less links are congested and the severity of congestions is lower.
...
Further investigation can be conducted to assess how the algorithm performs in heterogeneous scenarios. That is, it would be interesting to investigate whether the RL drivers can adapt themselves to the greedy drivers or even the ones using the minority game algorithm proposed by \cite{Galib&Moser2011}. Future work can also attempt to assess how good it would be for agents when they consider other agents, that is, how good it would be to learn joint actions in this competitive environment.
